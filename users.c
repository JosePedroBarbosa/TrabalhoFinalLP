...
void rankCompanies(Empresas *empresas) {
    ...
    for (int i = 0; i < empresas->contador; i++) {
        if (strcmp(empresas->empresas[i].nome, companyName) == 0) {
            ...
            //if company without rating starts rating with value of 0.0
            if (empresas->empresas[i].nClassis == 0) {
                empresas->empresas[i].classis->nota = 0.0;
            }

            if (empresas->empresas[i].nClassis == INITIAL_CLASSIS) {
                empresas->empresas[i].classis = realloc(empresas->empresas[i].classis, sizeof(Classificacao) * (empresas->empresas[i].nClassis + INITIAL_CLASSIS));
...
void commentCompanies(Empresas *empresas){
    char companyName[MAX_COMPANY_NAME_SIZE];
    char title[MAX_TITLE_SIZE], text[MAX_TEXT_SIZE], userName[MAX_NAME_SIZE], userEmail[MAX_EMAIL_SIZE];
    ...
    for(int i = 0; i < empresas->contador; i++){
        if(strcmp(empresas->empresas[i].nome, companyName) == 0){
            ...
            do{
                readString(userName, MAX_NAME_SIZE, MSG_GET_USER_NAME);
            ...
            do{
                readString(title, MAX_TITLE_SIZE, MSG_GET_COMMENT_TITLE);

            } while(validateString(title) != 1);

            do{
                readString(text, MAX_TEXT_SIZE, MSG_GET_COMMENT_TEXT);

            } while(validateString(text) != 1);

            if (empresas->empresas[i].nComments == INITIAL_COMMENTS) {
                empresas->empresas[i].comments = realloc(empresas->empresas[i].comments, sizeof(Comentario) * (INITIAL_COMMENTS * 2));
            ...
            empresas->empresas[i].comments[empresas->empresas[i].nComments].estado = 1;
